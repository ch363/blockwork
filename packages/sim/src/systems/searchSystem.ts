 ...
 *
 * Contraband storage uses {@link ContrabandStub} until T4.2's full system
 * lands; search only needs carried items and tile stashes.
 */
...
export interface MisconductStandingOrder {
  ...
  /**
   * Hours of lockdown / isolation. `0` means n/a (typically ignore).
   * `-1` means indefinite.
   */
...
export interface StandingOrdersPolicy {
  readonly responses: Record<MisconductKind, MisconductStandingOrder>
  reassignmentStrictness: ReassignmentStrictness
  meals: MealStandingOrders
}
...
export function createStandingOrdersPolicy(data: GameData): StandingOrdersPolicy {
  ...
  const responses = {} as Record<MisconductKind, MisconductStandingOrder>
  for (const kind of MISCONDUCT_KINDS) {
    const entry = defaults[kind]
    if (entry === undefined) {
      responses[kind] = { punishment: 'ignore', durationHours: 0, search: false }
    } else {
      responses[kind] = {
  ...
  return {
    ...
    meals: {
      quantity: data.balance.kitchen.defaultMealQuantity,
      variety: data.balance.kitchen.defaultMealVariety,
    },
  }
}
...
export function performSearch(options: PerformSearchOptions): SearchResult {
  ...
  if (kind === 'individual') {
    ...
    for (const [tileIndex] of world.contraband.stashes) {
    ...
    for (const tileIndex of [...world.contraband.stashes.keys()]) {
      const taken = world.contraband.searchStash(tileIndex, chance, () => rng.chance(chance))
      if (taken.length > 0) {
        found.push(...taken)
        stashesTouched += 1
...
export function rollMetalDetectorPass(options: PassiveDetectOptions): string[] {
  ...
  const bag = [...world.contraband.carriedOf(inmateId)]
  const taken: string[] = []
...
function removeSpecificCarried(
  ledger: ContrabandStub,
  inmate: InmateEntity['inmate'],
  inmateId: number,
  remove: readonly string[],
): void {
  const remaining = [...(ledger.carried.get(inmateId) ?? [])]
  const next: string[] = []
  ...
  if (next.length === 0) ledger.carried.delete(inmateId)
  else ledger.carried.set(inmateId, next)
  ...
  return {
    ...
    update(context: SystemContext): void {
      ...
      for (const entity of world.inmates.all()) {
        ...
        if (world.contraband.carriedOf(entity.id).length === 0) continue
...
export function searchCommandHandlers(data: GameData): Record<string, CommandHandler> {
  ...
  return {
    ...
    [SEARCH_COMMANDS.setPunishment]: (command, context) => {
      ...
      const order = context.world.standingOrders.responses[misconduct]
    ...
    [SEARCH_COMMANDS.setSearchTrigger]: (command, context) => {
      ...
      context.world.standingOrders.responses[misconduct].search = search
    },
    ...
    [SEARCH_COMMANDS.setMeals]: (command, context) => {
      ...
      const meals: MealStandingOrders = {
        quantity: quantity as MealQuantity,
        variety,
      }
      context.world.standingOrders.meals = meals
      context.world.meals.standingOrders.quantity = meals.quantity
      context.world.meals.standingOrders.variety = meals.variety
    },
  }
}
