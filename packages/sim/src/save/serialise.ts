...
export function toSaveFile(state: SaveState, options: SaveOptions): SaveFile {
  ...
  return {
    ...
    intake: state.intake,
    cellGrades: state.cellGrades,
    incomeOwed: state.incomeOwed,
    staffOnlyRoomIds: state.staffOnlyRoomIds,
    intakeSearchedInmateIds: state.intakeSearchedInmateIds,
    staffNeedsEnabled: state.staffNeedsEnabled,
    fog: state.fog,
    ...
    routineRuntime: state.routineRuntime,
    meals: state.meals,
    supply: state.supply,
